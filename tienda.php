...
<html>
      ...
      ?>
      <meta charset="utf-8">
    	<title>Digital MTX | Tienda Online</title>
                ...
                <div class="col s12 filtros-wrapper">
                    <div class="col s12 valign-wrapper">
                        <form action="" method="GET" class="col s12">
                            <div class="col s12 m2 center filtros-titulo-wrapper">
                                <?php
                            ...
                            <div class="col s12 m2">
                                <div class="input-field col s12" style="margin-top: -20px;">
                                    <select class="browser-default" name="descuento">
                                        <option value="" disabled selected>Descuento</option>
                                        <?php
                                            $opciones_descuento = array(
                                                '4-Sin descuento',
                                                '1-Con descuento'
                                            );
                                            for ($i=0; $i < count($opciones_descuento); $i++) { 
                                                $datos = explode('-', $opciones_descuento[$i]);
                                                if (isset($_GET['descuento']) && $_GET['descuento'] == $datos[0]) {
                                                    ?>
                            ...
                            <div class="col s12 m2 center"  style="margin-top: -11px;">
                                <?php
                                    if (isset($_GET['filtrar']) || isset($_GET['categoria']) || !empty($_GET['like']) || !empty($_GET['tags'])) {
                                        ?>
                                            <a href="tienda.php" class="btn btn-eliminar-filtro center waves-effect waves-light tooltipped" data-position="bottom" data-tooltip="Quitar filtros"><i class="material-icons">delete</i></a>
                                        <?php
                                    }
                                ?>
                ...
                <div class="col s12 productos-wrapper">
                    <?php
                        if(isset($_GET['filtrar']) || isset($_GET['categoria'])) {
                            ...
                            $qryadd="SELECT * FROM dtm_productos";
                            if(!empty($_GET['categoria'])) {
                                $categoria=$_GET['categoria'];
                                $qryadd .=" WHERE categoria='$categoria' AND estado ='4'";
                            }
                            ...
                            if(!empty($_GET['categoria']) && !empty($_GET['marca'])) {
                                ...
                                $marca=$_GET['marca'];
                                $qryadd .=" WHERE marca='$marca' AND estado ='4'";
                            }
                 
                            if(empty($_GET['categoria']) && empty($_GET['marca']) && !empty($_GET['descuento'])) {
                                if($_GET['descuento']==4) {
                                    $qryadd .=" WHERE descuento='0' AND estado ='4'";
